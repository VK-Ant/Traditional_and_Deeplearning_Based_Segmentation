...
# -*- coding: utf-8 -*-
"""
Created on Sun Dec 18 10:46:33 2022
...
1. Images come in many size: 797x644,509x544, 682x658, 1099x846,1126x1058, 859x838, 1817x2061, 2149x1479

2. Need to preprocess, so we can capture all images into numpy arrays. crop to a size divisible by 256 and extract patches

Notice: Not recommended to reshape

3. Masks are rgb and information provided as hex color code.
#Need to convert hex to rgb values and then convert rgb labels to integer values and then to one hot encoded.

3.1 Build the model

4. Predicted segmented images need to converted back into original rgb color

5. Predicted tiles need to be merged into a large image by minimizing blending artefacts(smooth blending)

"""
"""
Classes and representing color
...
1. Read images from repsective images subdirectory
2. As all images are of different size we have 2 options, either resize or crop
3. But, some images are too large and some small. Resizing will change the size of real objects.
4. Therefore, we will crop them to a nearest size divisible by 256 and then divide all images into patches of  256*256*3
